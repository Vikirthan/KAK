...
export default function LoginPage() {
    ...
    // Debug state for Google login/session
    const [debugInfo, setDebugInfo] = useState<{ session: any, code: string | null, error: string | null }>({ session: null, code: null, error: null });
    ...
    useEffect(() => {
        const hydrateGoogleSession = async () => {
            ...
            try {
                ...
                // Try to get session object from supabase directly if available
                if (window.supabase) {
            ...
            // Try to extract OAuth code from URL
            try {
                ...
                debugError = debugError || e?.message || String(e);
            }
            setDebugInfo({ session, code, error: debugError });
        };
    ...
    return (
        <div className="min-h-screen bg-[#0a0a14] flex items-center justify-center p-4 font-['Inter',sans-serif] relative overflow-hidden">
            ...
            <div style={{position:'fixed',top:0,left:0,right:0,zIndex:9999}}>
                <div style={{background:'#222',color:'#fff',padding:'8px',fontSize:'12px',borderBottom:'2px solid #f90'}}>
                    <b>DEBUG:</b> Session: {debugInfo.session ? 'YES' : 'NO'} | OAuth code: {debugInfo.code || 'none'} | Error: {debugInfo.error || 'none'}
                </div>
            </div>
            {/* Animated background */}
